...
def init(project_path):
    ...
    for root, dirs, files in os.walk(project_path):
        for file in files:
            ...
            project_data["functions"][file_path] = parse_all_function_info(c_file_content, tree)
...
def query_function(function_name: str) -> str:
    ...
    for file_path, fun_info in file_to_fundef.items():
        if global_vars.project_path in file_path and function_name in fun_info:
...
def query_type(type_name: str) -> str:
    ...
    for file_path, type_info in file_to_typedef.items():
        if global_vars.project_path in file_path and type_name in type_info:
...
def query_def(def_name:str) -> str:
    ...
    for file_path, define_info in file_to_def.items():
        if global_vars.project_path in file_path and def_name in define_info:
...
############# Write   
def comment_out_main_function(code, main_node):
    lines = code.split('\n')
    start_line = main_node.start_point[0]
    end_line = main_node.end_point[0]
    # Commenting the lines containing the main function
    for i in range(start_line, end_line + 1):
        lines[i] = '// ' + lines[i]
    return '\n'.join(lines)

def add_new_main_function(code, new_main_code):

    return code + '\n' + new_main_code

def modify_main_function(new_main_code):
    file_to_fundef, _, _ = init(global_vars.project_path)
    for file in file_to_fundef:
        source_code, fun_info = file_to_fundef[file]
        if "main" in fun_info:     
            main_function_node = fun_info["main"]

            # Comment out the existing main function
            code = comment_out_main_function(source_code, main_function_node)

            # Add a new main function
            code = add_new_main_function(code, new_main_code)

            with open(file, 'w') as file:
                file.write(code)

def remove_function_definition(fun):
    file_to_fundef, _, _ = init(global_vars.project_path)

    for file in file_to_fundef:
        code, fun_info = file_to_fundef[file]

        # Check if the function exists in this file
        if fun in fun_info:     
            function_node = fun_info[fun]
            lines = code.split('\n')
            ...
            modified_lines = lines[:start_line] + lines[end_line+1:]

            # Write the modified file
            with open(file, 'w') as f:
                f.write("\n".join(modified_lines))

            print(f"Removed function 'parse_bfd_packet_buggy' from {file}")
